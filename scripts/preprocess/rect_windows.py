...
def main():
    parser = argparse.ArgumentParser(
        description="Create rectangular fixed-length BVP windows from final preprocessed PKL files."
    )
    parser.add_argument(
        "--preprocessed-dir",
        type=Path,
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "data" / "windowed" / "bvp_360s_stride30s_rect",
        help="Directory to write rectangular subject NPZ window files.",
    )
    parser.add_argument(
        "--window-seconds",
        type=int,
